...
# probsbility of a customer sitting at a new table is 1/(n+1) if they are the nth customer
...
new_table_probability <- function(denom, alpha = 1) { # denom = tot_cust + 1 1 for new cust
  return (alpha / (denom + alpha - 1)) # always 1 / (n+1)
}

update_old_table_probability <- function(numbers, denom, alpha = 1) {
  return(numbers / (denom + alpha - 1))
}
...
for (n in 2:N_cust) {
  ntp <- new_table_probability(n, alpha = alpha_value)
  otp <- update_old_table_probability(numbers, n, alpha = alpha_value)
